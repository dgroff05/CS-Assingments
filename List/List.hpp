/*
  Filename   : List.hpp
  Author(s)  : William Killian, Gary M. Zoppetti, Dylan Groff
  Course     : CSCI 362-01
  Date       : 3/25/25
  Assignment : List
...
struct ListNode
{
  ...
  unhook_range (ListNode* begin, ListNode* end)
  {
    ...
    ListNode* first = begin -> prev;

    // Splices out range
    if (last != nullptr) last -> prev = first;
    if (first != nullptr) first -> next = last;

    // Gets rid of range's links to old nodes
    begin -> prev = nullptr;
    end -> next = nullptr;  
  }
...
public: /* should be private, but public for testing */
  ...
  transfer (iterator pos, iterator first, iterator last)
  {
    ...
    Node::unhook_range (first.m_nodePtr, last.m_nodePtr);
    
    (*pos.m_nodePtr).hook_range(first.m_nodePtr, last.m_nodePtr);
    // Node::unhook_range(...);
    // Node::hook_range(...);
...
public:
  ...
  operator= (const List& other)
  {
    ...
    if (this != &other)
    {
      ...
      {
        List<T>::const_iterator iter1 = other.begin ();
        List<T>::iterator iter2 = begin ();
  ...
  {
    // TODO
    if (pos.m_nodePtr == nullptr) return pos;

    // Gets the next element before the node pos points to is deleted
    if (pos == end ()) --pos;
    Node* nextElement = pos.m_nodePtr -> next;
  ...
  reverse ()
  {
    ...
    Node* firstNode = m_header.next;
    Node* secondNode = firstNode -> next;
    
    while (firstNode != &m_header)
    {
      // Swaps the links of the first node
    ...
    // Performs last set of link swaps
    firstNode -> next = firstNode -> prev;
    firstNode -> prev = secondNode;
  }
